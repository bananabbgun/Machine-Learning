...
def sce(
    ...
    """
    if not task_tensors:
        raise ValueError("task_tensors cannot be empty")
    
    # S: Stack all task tensors into a single tensor of shape [num_tasks, ...]
    task_tensors_stacked = torch.stack(task_tensors, dim=0)
    ...
    if density < 1.0:
        ...
        mask = sce_mask(task_tensors_stacked, density, mask_dtype=task_tensors_stacked.dtype)
        # Apply the binary mask across all task tensors.
        task_tensors_stacked = task_tensors_stacked * mask.unsqueeze(0)
    
    # E: Compute a binary mask indicating majority sign agreement per element across task vectors. (reuse)
    majority_sign_mask = calculate_majority_sign_mask(task_tensors_stacked, method=majority_sign_method)
    
    # C: Compute task-specific weights with sce_weight()
    task_weights = sce_weight(task_tensors_stacked)
    
    # Reshape weights to match dimensions of task_tensors for broadcasting.
    task_weights = reshape_weight_task_tensors(task_tensors_stacked, task_weights)
    ...
    weighted_task_tensors = task_tensors_stacked * task_weights * majority_sign_mask
    
    # Weighted summation of masked task tensors to create a merged task tensor.
    merged_tensor = weighted_task_tensors.sum(dim=0)
    ...
    weight_sum = (task_weights * majority_sign_mask).sum(dim=0)
    normalized_tensor = merged_tensor / torch.clamp(weight_sum, min=1e-6)
...
def sce_mask(task_tensors: torch.Tensor, density: float, mask_dtype: Optional[torch.dtype] = None):
    ...
    if density <= 0:
        return torch.zeros_like(task_tensors[0], dtype=mask_dtype)  # 修正：使用 task_tensors[0]
    
    # If density is one, keep everything
    if density >= 1:
        return torch.ones_like(task_tensors[0], dtype=mask_dtype)   # 修正：使用 task_tensors[0]
